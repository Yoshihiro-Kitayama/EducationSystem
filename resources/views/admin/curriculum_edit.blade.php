...
<body>
    <div class="container">
        <div class="row">
            <a href="{{ route('admin.curriculum.list') }}" class="col-1 btn btn-secondary btn-sm">← 戻る</a>
            <h2 class="my-3">授業設定</h2>
        </div>
        <main class="row">
            <form action="{{ route('admin.curriculum.update', ['curriculums_id' => $curriculums_id]) }}" method="POST" enctype="multipart/form-data" >
                @csrf
                @if ($errors->any())
